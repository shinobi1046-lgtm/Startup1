...
async function generateWorkflowFromAnalysis(analysis: AIAnalysisResult, originalPrompt: string) {
  // Build workflow structure from AI analysis
  const nodes = [];
  const connections = [];
  
  // Create nodes based on AI-detected apps
  analysis.requiredApps.forEach((app, index) => {
    const nodeId = `${app.toLowerCase().replace(/\s+/g, '-')}-${index}`;
    
    nodes.push({
      id: nodeId,
      type: app.toLowerCase().replace(/\s+/g, '-'),
      app: app,
      function: getFunctionForApp(app, originalPrompt),
      parameters: getParametersForApp(app, originalPrompt, analysis),
      position: { x: 100 + (index * 200), y: 100 + (index % 2) * 100 },
      icon: getIconForApp(app),
      color: getColorForApp(app)
    });
  ...
  const appsScriptCode = generateAppsScriptCode(nodes, analysis);
