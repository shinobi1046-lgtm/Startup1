...
export function registerAIWorkflowRoutes(app: express.Application) {
  ...
  app.post('/api/ai/generate-workflow', async (req, res) => {
    ...
    try {
      ...
      if (hasAnswers) {
        // Always use AI analysis for workflow generation
        console.log('🧠 Using AI-powered workflow generation with user answers');
        
        // First get AI analysis of the prompt
        try {
          analysis = await MultiAIService.analyzeWorkflowPrompt(enhancedPrompt);
          workflow = await buildIntelligentWorkflowFromAnalysis(analysis, answers, prompt);
        } catch (error) {
          console.log('⚠️ AI analysis failed, using intelligent pattern matching');
          workflow = await buildIntelligentWorkflowFromAnswers(answers, prompt);
          analysis = {
            modelUsed: 'Intelligent Pattern Matcher',
            processingTime: Date.now() - Date.now(),
            confidence: 0.85,
            intent: workflow.intelligence?.intent || 'automation',
            complexity: workflow.complexity || 'Complex',
            reasoning: 'Built using intelligent pattern matching with user answers'
          };
        }
      } else {
        analysis = await MultiAIService.analyzeWorkflowPrompt(enhancedPrompt);
...
async function buildIntelligentWorkflowFromAnalysis(analysis: any, answers: any, originalPrompt: string) {
  console.log('🎯 Building workflow from AI analysis:', analysis.intent);
  
  // Extract common info from answers
  const { sheetId, sheetName, emailContent } = extractCommonInfo(answers);
  
  // Map AI analysis to workflow nodes based on intent and required apps
  const workflowBuilder = new IntelligentWorkflowBuilder(analysis, answers, originalPrompt);
...
class IntelligentWorkflowBuilder {
  ...
  buildWorkflow() {
    const { sheetId, sheetName, emailContent } = extractCommonInfo(this.answers);
    
    // Determine workflow type from AI analysis
    const workflowType = this.detectWorkflowTypeFromAnalysis();
    console.log(`🎯 AI-detected workflow type: ${workflowType}`);
    
    // Build nodes based on AI analysis
    const nodes = this.buildNodesFromAnalysis(workflowType);
    const connections = this.buildConnectionsFromNodes(nodes);
    const appsScriptCode = this.generateCodeForWorkflow(workflowType);
    
    return {
      id: `workflow-${Date.now()}`,
      title: generateIntelligentTitle(this.analysis.intent, this.originalPrompt),
      description: this.generateDescription(workflowType),
      nodes,
      connections,
      appsScriptCode,
      ...
      intelligence: {
        ...
        confidence: this.analysis.confidence || 0.90,
  ...
  private detectWorkflowTypeFromAnalysis(): string {
    const intent = this.analysis.intent?.toLowerCase() || '';
    const prompt = this.originalPrompt.toLowerCase();
    const apps = this.analysis.requiredApps || [];
    
    // Map AI intent to workflow types
    if (intent.includes('interview') || intent.includes('recruit') || intent.includes('candidate')) {
      return 'interview_automation';
    }
    
    if (intent.includes('backup') || intent.includes('sync') || intent.includes('copy')) {
      return 'data_sync';
    }
    
    if (intent.includes('report') || intent.includes('crm') || intent.includes('slack')) {
      return 'api_integration';
    }
    
    if (intent.includes('email') || apps.includes('Gmail')) {
      return 'email_automation';
    }
    
    if (intent.includes('sheet') || apps.includes('Google Sheets')) {
      return 'sheet_automation';
    }
    
    // Default based on most common app
    if (apps.includes('Google Drive') || apps.includes('Dropbox')) {
  ...
  private buildNodesFromAnalysis(workflowType: string) {
    const { sheetId, sheetName, emailContent } = extractCommonInfo(this.answers);
    const apps = this.analysis.requiredApps || [];
    
    switch (workflowType) {
      case 'interview_automation':
        return this.buildInterviewNodes(sheetId, sheetName, emailContent);
      
      case 'data_sync':
        return this.buildDataSyncNodes(apps);
      
      case 'api_integration':
        return this.buildAPIIntegrationNodes(apps);
      
      case 'file_management':
        return this.buildFileManagementNodes(apps);
      
      case 'email_automation':
        return this.buildEmailNodes(emailContent);
      
      case 'sheet_automation':
        return this.buildSheetNodes(sheetId, sheetName);
      
      default:
        return this.buildGenericNodes();
    }
  }
  
  private buildInterviewNodes(sheetId: string, sheetName: string, emailContent: string) {
    return [
      {
        id: 'sheets-trigger',
        type: 'google-sheets',
      ...
      },
      {
        ...
        parameters: { statusColumn: 'D', condition: 'empty' },
        position: { x: 380, y: 80 },
        icon: '🔍',
        color: '#9AA0A6',
  ...
  private buildDataSyncNodes(apps: string[]) {
    return [
      {
        id: 'data-fetch',
        type: apps[0]?.toLowerCase().replace(/\s+/g, '-') || 'api',
        app: apps[0] || 'Data Source',
        function: 'fetch_data',
        functionName: 'Fetch Source Data',
        parameters: { source: apps[0] || 'Source' },
        position: { x: 120, y: 80 },
        icon: '📥',
        color: '#4285F4',
        aiReason: `Retrieves data from ${apps[0] || 'source system'}`,
        confidence: 0.85,
        isRequired: true
      },
      {
        ...
        parameters: { action: 'format_and_validate' },
        position: { x: 380, y: 80 },
        icon: '🔄',
        color: '#FF6D01',
        aiReason: 'Processes and transforms data for target system',
        confidence: 0.80,
        isRequired: true
      },
      {
        id: 'data-sync',
        type: apps[1]?.toLowerCase().replace(/\s+/g, '-') || 'api',
        app: apps[1] || 'Data Target',
        function: 'sync_data',
        functionName: 'Sync to Target',
        parameters: { target: apps[1] || 'Target' },
        position: { x: 640, y: 80 },
        icon: '📤',
        color: '#34A853',
        aiReason: `Syncs processed data to ${apps[1] || 'target system'}`,
        confidence: 0.85,
        isRequired: true
      }
    ];
  ...
  private buildAPIIntegrationNodes(apps: string[]) {
    return [
      {
        id: 'api-trigger',
        type: 'timer',
        app: 'Built-in',
        function: 'schedule_trigger',
        functionName: 'Schedule Trigger',
        parameters: { frequency: 'weekly' },
        position: { x: 120, y: 80 },
        icon: '⏰',
        color: '#9AA0A6',
        aiReason: 'Triggers the workflow on a scheduled basis',
        confidence: 0.90,
        isRequired: true
      },
      {
        id: 'api-fetch',
        type: apps[0]?.toLowerCase().replace(/\s+/g, '-') || 'api',
        app: apps[0] || 'API Source',
        function: 'fetch_api_data',
        functionName: 'Fetch API Data',
        parameters: { endpoint: 'api_endpoint' },
        position: { x: 380, y: 80 },
        icon: '🔌',
        color: '#4285F4',
        aiReason: `Fetches data from ${apps[0] || 'API source'}`,
        confidence: 0.80,
        isRequired: true
      },
      {
        id: 'api-send',
        type: apps[1]?.toLowerCase().replace(/\s+/g, '-') || 'api',
        app: apps[1] || 'API Target',
        function: 'send_api_data',
        functionName: 'Send to Target',
        parameters: { channel: 'target_channel' },
        position: { x: 640, y: 80 },
        icon: '📨',
        color: '#34A853',
        aiReason: `Sends processed data to ${apps[1] || 'target system'}`,
        confidence: 0.80,
        isRequired: true
      }
    ];
  ...
  private buildFileManagementNodes(apps: string[]) {
    return [
      {
        id: 'file-scan',
        type: apps[0]?.toLowerCase().replace(/\s+/g, '-') || 'drive',
        app: apps[0] || 'File Source',
        function: 'scan_files',
        functionName: 'Scan Source Files',
        parameters: { folder: 'source_folder' },
        position: { x: 120, y: 80 },
        icon: '🔍',
        color: '#4285F4',
        aiReason: `Scans files in ${apps[0] || 'source location'}`,
        confidence: 0.85,
        isRequired: true
      },
      {
        id: 'file-copy',
        type: apps[1]?.toLowerCase().replace(/\s+/g, '-') || 'drive',
        app: apps[1] || 'File Target',
        function: 'copy_files',
        functionName: 'Copy Files',
        parameters: { destination: 'backup_folder' },
        position: { x: 380, y: 80 },
        icon: '📋',
        color: '#34A853',
        aiReason: `Copies files to ${apps[1] || 'backup location'}`,
        confidence: 0.85,
        isRequired: true
      }
    ];
  ...
  private buildEmailNodes(emailContent: string) {
    return [
      {
        id: 'gmail-trigger',
        type: 'gmail',
      ...
      },
      {
        ...
        parameters: { bodyTemplate: emailContent || 'Thank you for your email.' },
        position: { x: 380, y: 80 },
        icon: '↩️',
        color: '#EA4335',
  ...
  private buildSheetNodes(sheetId: string, sheetName: string) {
    return [
      {
        id: 'sheets-trigger',
        type: 'google-sheets',
    ...
    return [
      {
        ...
        functionName: 'Manual Trigger',
        parameters: {},
        position: { x: 120, y: 80 },
        icon: '🔘',
        color: '#9AA0A6',
        aiReason: 'Manually triggered automation',
        confidence: 0.70,
        isRequired: true
      },
      {
        ...
        icon: '⚡',
        color: '#FF6D01',
        aiReason: 'Performs custom automation action',
        confidence: 0.70,
        isRequired: true
      }
    ];
  ...
  private buildConnectionsFromNodes(nodes: any[]) {
    const connections = [];
    for (let i = 0; i < nodes.length - 1; i++) {
      connections.push({
        ...
        source: nodes[i].id,
        target: nodes[i + 1].id,
        dataType: 'workflow_data'
      });
    }
    return connections;
  }
  
  private generateCodeForWorkflow(workflowType: string): string {
    const { sheetId, sheetName, emailContent } = extractCommonInfo(this.answers);
    
    switch (workflowType) {
      case 'interview_automation':
        return generateInterviewAutomationGAS({ sheetId, sheetName, emailContent });
      case 'data_sync':
        return generateDataSyncGAS({ intent: this.analysis.intent }, { sheetId, sheetName, emailContent });
      case 'api_integration':
        return generateAPIIntegrationGAS({ intent: this.analysis.intent }, { sheetId, sheetName, emailContent });
      case 'file_management':
        return generateFileManagementGAS({ intent: this.analysis.intent }, { sheetId, sheetName, emailContent });
      default:
        return generateGenericGAS({ emailContent });
    }
  }
  
  private generateDescription(workflowType: string): string {
    switch (workflowType) {
      case 'interview_automation':
        return 'Automatically sends interview invitations when new candidates are added to spreadsheet. Tracks status in column D.';
      case 'data_sync':
        return 'Synchronizes data between applications on a scheduled basis.';
      case 'api_integration':
        return 'Integrates multiple systems via API calls and data transfer.';
      case 'file_management':
        return 'Manages files across different storage systems and platforms.';
      case 'email_automation':
        return 'Automates email processing and responses.';
      case 'sheet_automation':
        return 'Monitors and processes spreadsheet changes automatically.';
      default:
        return 'Custom automation workflow based on user requirements.';
    }
  }
}

async function buildIntelligentWorkflowFromAnswers(answers: any, originalPrompt: string) {
  console.log('🧠 Building intelligent workflow using AI analysis...');
  
  try {
    // Use Gemini to analyze the prompt and answers to determine workflow structure
    const analysisPrompt = `Analyze this automation request and determine the appropriate workflow nodes:

USER REQUEST: "${originalPrompt}"
USER ANSWERS: ${JSON.stringify(answers, null, 2)}

Based on this request, determine:
1. What type of automation this is (e.g., data_sync, email_automation, file_management, api_integration, etc.)
2. What applications/services are involved
3. What the logical flow should be (trigger → processing → actions)
4. What specific functions each node should perform

Return a JSON object with this structure:
{
  "workflowType": "specific_type",
...
Be specific about the actual functions needed, not generic placeholders.`;

    const apiKey = process.env.GEMINI_API_KEY;
    console.log('🔑 Checking Gemini API key availability:', apiKey ? 'Available' : 'Missing');
    if (!apiKey) {
      console.log('⚠️ No Gemini API key, falling back to pattern-based detection');
      return buildWorkflowFromAnswers(answers, originalPrompt);
    }
    ...
    if (!response.ok) {
      console.log('⚠️ Gemini API failed:', response.status, response.statusText);
      const errorText = await response.text();
      console.log('⚠️ API Error details:', errorText);
    ...
    if (!aiResponse) {
      console.log('⚠️ No AI response, falling back to pattern-based detection');
      return buildWorkflowFromAnswers(answers, originalPrompt);
    }
    ...
    if (!jsonMatch) {
      console.log('⚠️ No valid JSON in AI response, falling back to pattern-based detection');
      return buildWorkflowFromAnswers(answers, originalPrompt);
    }

    const aiWorkflow = JSON.parse(jsonMatch[0]);
    console.log('🤖 AI-generated workflow structure:', aiWorkflow);

    // Convert AI analysis to our workflow format
    return convertAIWorkflowToFormat(aiWorkflow, answers, originalPrompt);
...
function convertAIWorkflowToFormat(aiWorkflow: any, answers: any, originalPrompt: string) {
  console.log('🔄 Converting AI workflow to our format...');
  
  // Extract common info from answers for parameter filling
  const { sheetId, sheetName, emailContent } = extractCommonInfo(answers);
  
  // Convert AI nodes to our node format
  const nodes = aiWorkflow.flow.map((aiNode: any, index: number) => ({
    id: aiNode.id,
    type: aiNode.app.toLowerCase().replace(/\s+/g, '-'),
    app: aiNode.app,
    function: aiNode.function,
    functionName: aiNode.functionName,
    parameters: {
      ...aiNode.parameters,
      // Fill in extracted parameters where relevant
      ...(sheetId && aiNode.app.includes('Sheets') ? { spreadsheetId: sheetId, sheetName } : {}),
      ...(emailContent && aiNode.app.includes('Gmail') ? { emailContent } : {})
    },
    position: { x: 120 + (index * 260), y: 80 },
    icon: getAppIcon(aiNode.app),
    color: getAppColor(aiNode.app),
    aiReason: aiNode.description,
    confidence: 0.90,
    isRequired: true
  }));

  // Convert AI connections to our format
  const connections = aiWorkflow.connections.map((conn: any, index: number) => ({
    id: `c${index + 1}`,
    source: conn.from,
    target: conn.to,
    dataType: 'workflow_data'
  ...
  // Generate appropriate Apps Script code based on workflow type
  const appsScriptCode = generateIntelligentGAS(aiWorkflow, { sheetId, sheetName, emailContent });

  return {
    id: `workflow-${Date.now()}`,
    title: generateIntelligentTitle(aiWorkflow.intent, originalPrompt),
    ...
    complexity: nodes.length > 3 ? 'Complex' : 'Medium',
    intelligence: {
      intent: aiWorkflow.workflowType,
      confidence: 0.90,
      logicalFunctions: nodes.map(n => ({
        app: n.app,
        function: n.function,
...
function extractCommonInfo(answers: any) {
  ...
  return { sheetId, sheetName, emailContent };
}
...
function generateIntelligentGAS(aiWorkflow: any, config: any): string {
  const { sheetId, sheetName, emailContent } = config;
  
  // Generate code based on workflow type
  switch (aiWorkflow.workflowType) {
    case 'interview_automation':
      return generateInterviewAutomationGAS({ sheetId, sheetName, emailContent });
    case 'data_sync':
      return generateDataSyncGAS(aiWorkflow, config);
    case 'file_management':
      return generateFileManagementGAS(aiWorkflow, config);
    case 'api_integration':
      return generateAPIIntegrationGAS(aiWorkflow, config);
    default:
      return generateGenericGAS({ emailContent });
  }
}

function generateDataSyncGAS(aiWorkflow: any, config: any): string {
  return `/**
 * Data Sync Automation
 * Generated by AI Builder for ${aiWorkflow.intent}
 */
...
function syncData() {
  try {
    ...
    // This is a template - specific implementation depends on APIs available
    // For Google Apps Script, you can use:
    // - UrlFetchApp for API calls
    // - SpreadsheetApp for Sheets
    // - DriveApp for Drive files
    // - GmailApp for emails
...
function generateFileManagementGAS(aiWorkflow: any, config: any): string {
  return `/**
...
function manageFiles() {
  try {
    console.log('📁 Starting file management automation...');
    
    // Example: Backup files from Google Drive
    ...
    console.log('✅ File management completed successfully');
...
function generateAPIIntegrationGAS(aiWorkflow: any, config: any): string {
  return `/**
 * API Integration Automation
 * Generated by AI Builder for ${aiWorkflow.intent}
 */

function integrateAPIs() {
  try {
    console.log('🔌 Starting API integration automation...');
    
    // Example API call using UrlFetchApp
    ...
    // Process the data (e.g., save to Sheets)
    // const sheet = SpreadsheetApp.openById('SHEET_ID').getActiveSheet();
    // sheet.appendRow([data.field1, data.field2, new Date()]);
