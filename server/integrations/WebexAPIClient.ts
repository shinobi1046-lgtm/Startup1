// WEBEX API CLIENT
// Auto-generated API client for Webex integration

import { BaseAPIClient } from './BaseAPIClient';

export interface WebexAPIClientConfig {
  accessToken: string;
  baseUrl?: string;
}

export class WebexAPIClient extends BaseAPIClient {
  protected baseUrl: string;
  private config: WebexAPIClientConfig;

  constructor(config: WebexAPIClientConfig) {
    super();
    this.config = config;
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/people/me');
