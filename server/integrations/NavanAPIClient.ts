// NAVAN (TRIPACTIONS) API CLIENT
// Auto-generated API client for Navan (TripActions) integration

import { BaseAPIClient } from './BaseAPIClient';

export interface NavanAPIClientConfig {
  apiKey: string;
  baseUrl?: string;
}

export class NavanAPIClient extends BaseAPIClient {
  protected baseUrl: string;
  private config: NavanAPIClientConfig;

  constructor(config: NavanAPIClientConfig) {
    super();
    this.config = config;
    this.baseUrl = config.baseUrl || 'https://api.tripsactions.com/v2';
  }
