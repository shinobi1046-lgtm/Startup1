// LOOKER API CLIENT
// Auto-generated API client for Looker integration

import { BaseAPIClient } from './BaseAPIClient';

export interface LookerAPIClientConfig {
  accessToken: string;
  refreshToken?: string;
  clientId?: string;
  clientSecret?: string;
...
export class LookerAPIClient extends BaseAPIClient {
  protected baseUrl: string;
  private config: LookerAPIClientConfig;

  constructor(config: LookerAPIClientConfig) {
    super();
    this.config = config;
  ...
  /**
   * Update an existing record in Looker
   */
  ...
  /**
   * Poll for Triggered when a new record is created in Looker
   */
  ...
  /**
   * Poll for Triggered when a record is updated in Looker
   */
