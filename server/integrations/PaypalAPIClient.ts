// PAYPAL API CLIENT
// Auto-generated API client for PayPal integration

import { BaseAPIClient } from './BaseAPIClient';

export interface PaypalAPIClientConfig {
  accessToken: string;
  baseUrl?: string;
}

export class PaypalAPIClient extends BaseAPIClient {
  protected baseUrl: string;
  private config: PaypalAPIClientConfig;

  constructor(config: PaypalAPIClientConfig) {
    super();
    this.config = config;
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/v2/checkout/orders?page_size=1');
