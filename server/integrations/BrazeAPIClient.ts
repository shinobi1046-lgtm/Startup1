// BRAZE API CLIENT
// Auto-generated API client for Braze integration
...
export class BrazeAPIClient extends BaseAPIClient {
  protected baseUrl: string;
  private config: BrazeAPIClientConfig;

  constructor(config: BrazeAPIClientConfig) {
    super();
    this.config = config;
    this.baseUrl = config.baseUrl || 'https://rest.iad-01.braze.com';
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/users/track');
