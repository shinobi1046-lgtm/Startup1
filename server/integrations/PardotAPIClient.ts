// SALESFORCE PARDOT (ACCOUNT ENGAGEMENT) API CLIENT
// Auto-generated API client for Salesforce Pardot (Account Engagement) integration

import { BaseAPIClient } from './BaseAPIClient';

export interface PardotAPIClientConfig {
  accessToken: string;
  baseUrl?: string;
}
...
export class PardotAPIClient extends BaseAPIClient {
  ...
  private config: PardotAPIClientConfig;

  constructor(config: PardotAPIClientConfig) {
    super();
    this.config = config;
    this.baseUrl = config.baseUrl || 'https://pi.pardot.com/api';
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/prospect/version/4/do/query?limit=1');
