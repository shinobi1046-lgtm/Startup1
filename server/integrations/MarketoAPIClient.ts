// MARKETO API CLIENT
// Auto-generated API client for Marketo integration

import { BaseAPIClient } from './BaseAPIClient';

export interface MarketoAPIClientConfig {
  accessToken: string;
  baseUrl?: string;
}
...
export class MarketoAPIClient extends BaseAPIClient {
  ...
  private config: MarketoAPIClientConfig;

  constructor(config: MarketoAPIClientConfig) {
    super();
    this.config = config;
    this.baseUrl = config.baseUrl || 'https://{{munchkin}}.mktorest.com/rest';
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/v1/leads.json?batchSize=1');
