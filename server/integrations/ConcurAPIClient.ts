...
// Auto-generated API client for SAP Concur integration

import { BaseAPIClient } from './BaseAPIClient';

export interface ConcurAPIClientConfig {
  accessToken: string;
  baseUrl?: string;
}

export class ConcurAPIClient extends BaseAPIClient {
  protected baseUrl: string;
  private config: ConcurAPIClientConfig;

  constructor(config: ConcurAPIClientConfig) {
    super();
    this.config = config;
    this.baseUrl = config.baseUrl || 'https://www.concursolutions.com/api';
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/userinfo');
