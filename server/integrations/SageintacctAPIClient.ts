...
// Auto-generated API client for Sage Intacct integration

import { BaseAPIClient } from './BaseAPIClient';

export interface SageintacctAPIClientConfig {
  accessToken: string;
  baseUrl?: string;
}
...
export class SageintacctAPIClient extends BaseAPIClient {
  ...
  private config: SageintacctAPIClientConfig;

  constructor(config: SageintacctAPIClientConfig) {
    super();
    this.config = config;
    this.baseUrl = config.baseUrl || 'https://api.intacct.com/ia/xml/xmlgw.phtml';
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/readByQuery');
