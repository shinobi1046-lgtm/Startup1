...
// Auto-generated API client for RingCentral integration

import { BaseAPIClient } from './BaseAPIClient';

export interface RingcentralAPIClientConfig {
  accessToken: string;
  baseUrl?: string;
}

export class RingcentralAPIClient extends BaseAPIClient {
  protected baseUrl: string;
  private config: RingcentralAPIClientConfig;

  constructor(config: RingcentralAPIClientConfig) {
    super();
    this.config = config;
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.makeRequest('GET', '/account/~/extension/~');
