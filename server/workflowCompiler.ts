...
export class WorkflowCompiler {
  ...
  public async compileWorkflow(graph: NodeGraph): Promise<{ files: CodeFile[]; entry: string }> {
    console.log('🔨 Compiling NodeGraph to Google Apps Script:', graph.name);
    ...
    console.log('✅ Compiled workflow into', files.length, 'files');
    return {
      files,
      entry: 'Code.js'
    };
...
function doPost(e) {
    ...
    return ContentService
      ...
      .createTextOutput(JSON.stringify({ success: false, error: getErrorMessage(error) }))
    return code;
  ...
  for (var attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        throw error;
      console.warn('HTTP request error (attempt ' + attempt + '), retrying:', getErrorMessage(error));
      Utilities.sleep(retryDelay * attempt);
